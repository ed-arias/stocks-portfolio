...
### Added
- **Customizable holdings table columns** (feature 2.8) — "Columns ⌄" button in the holdings header opens a dropdown with checkboxes to show/hide any of the 8 optional columns (Ticker is always visible); preferences persisted to `localStorage` under `holdings-visible-columns` and restored on page load; missing keys default to visible
