# analyst-consensus-rating-column Specification

## Purpose
Adds an optional "Analyst Rating" column to the holdings table that displays a color-coded consensus rating badge (label + analyst count) sourced from the backend. The column is visible by default, toggleable via the existing column picker, and sortable by rating tier using the existing tri-state sort system.
