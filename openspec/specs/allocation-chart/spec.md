# allocation-chart Specification

## Purpose
Generic reusable SVG donut chart component. Renders segments, a hover tooltip, and a color-coded legend from pre-computed backend data. Segment colors and legend labels are supplied by caller-provided `colorFn` and `labelFn` props — the component itself has no knowledge of asset classes, tickers, or any domain concept.
